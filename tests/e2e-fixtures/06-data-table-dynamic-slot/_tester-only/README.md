...
| 维度 | 关注 |
|---|---|
| CDP 契约落地 (50) | 三件套：`dynamic: true` + `dynamicSource: 'columns'` + `dynamicKey: '{key}'`；`scoped: true` + `scopeDescription`（含 record + index）；实现侧用 `_scopedSlots[col.key]?.({ record, index })`（**不**用 `_slots`）；`columns.dataIndex` 用 `format: 'dataField'`；columns 由 props 配置不写死 |
| 漏洞回避 (30) | 为每列硬编码 slot 名；漏 `dynamicSource` / `dynamicKey` / `scoped`；用 `_slots` 而非 `_scopedSlots`；`dynamicKey` 模板语法错（`:key` / `${key}`）；用 React render prop 自建 context；主动扩张 manifest 表面（排序/筛选/分页做成 manifest 暴露能力） |
